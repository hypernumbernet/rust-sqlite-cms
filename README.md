...
> **現状**: プロジェクトは設計・ドキュメント段階です。`cargo run` は Hello World のみ動作します。実装はロードマップに沿って進めます。
...
実装が進むと、初回起動時にマイグレーションが走り `data/cms.db` が作成される想定です。現時点では上記コマンドで Hello World のみが表示されます。
