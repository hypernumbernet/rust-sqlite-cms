...
pub async fn list_for_placeholder(pool: &SqlitePool, placeholder_id: i64) -> AppResult<Vec<Post>> {
    posts_repo::list_all_for_placeholder(pool, placeholder_id).await
}

/// ID で取得。
pub async fn find(pool: &SqlitePool, id: i64) -> AppResult<Post> {
    posts_repo::find(pool, id).await
}

/// 新規作成（image widget の postmeta もここで扱う想定）。
pub async fn create(pool: &SqlitePool, input: PostInput, meta: Option<std::collections::HashMap<String, String>>) -> AppResult<Post> {
...
/// 更新。
pub async fn update(pool: &SqlitePool, id: i64, input: PostInput, meta: Option<std::collections::HashMap<String, String>>) -> AppResult<()> {
