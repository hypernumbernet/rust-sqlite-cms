...
- メディアライブラリ、コメント、ユーザー管理などの管理画面（ナビゲーション上はリンクがありますが未実装）
...
`cargo run` で次の起動シーケンスが実行されます: 設定読み込み → `data/cms.db`（無ければ自動生成）への接続 → `migrations/`（0001-0003）の適用 → 既定 `options` の確認 → `work/` ディレクトリの初期化（テンプレート seed + pages ディレクトリ作成 + トップページ行の確保） → `127.0.0.1:3000` で待受。
...
- 画像カルーセル（Phase 2前倒し予定）、メディア・コメント・ユーザー・設定などの管理画面はナビゲーションにリンクがありますが、まだ未実装です（Phase 2 以降予定）
