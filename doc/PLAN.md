...
- `options` リポジトリと起動時の既定値投入（`blogname` / `blogdescription` / `siteurl` / `permalink_structure`）
...
flowchart TB
  ...
  subgraph infra [Infrastructure]
    ...
    MiniJinja["MiniJinja (work/templates・ランタイム)"]
...
- **theme / templates**: テンプレートのレンダリングと `work/` 同期（公開サイトは MiniJinja でランタイム評価、管理画面は Askama でコンパイル時検証）
...
│   ├── index.html           # 公開トップの初期テンプレート
│   └── landing.html / ...   # 新規ページ作成時の選択デザイン
├── work/                    # ステートフル作業ディレクトリ（.gitignore 対象）
│   └── templates/           # 公開テンプレート（MiniJinja・ランタイム差し替え）
│       ├── index.html       # /（無ければ presets から自動生成）
│       ├── page-{id}.html   # 管理画面で作成した各ページ
...
└── src/
    ...
    ├── db/                  # 接続・マイグレーション
    ├── models/
    ├── presets.rs           # 同梱デザインのレジストリ
    ├── repos/
    ├── theme/               # MiniJinja エンジン + work ディレクトリ同期
    ├── routes/
    │   ├── public.rs
    │   └── admin/
    └── templates/           # 管理画面用 Askama（公開テンプレートと分離）
        └── admin/
```
...
| `users` | ユーザーアカウント |
| `usermeta` | ユーザーメタ |
| `options` | サイト設定（`siteurl`, `blogname`, `permalink_structure` など） |
| `terms` | ターム名・スラッグ |
...
2. デフォルトテンプレートと公開ルート — `work/templates/index.html` と `/`・お知らせ詳細の表示
