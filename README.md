...
- 管理画面（`/admin/pages`）からページ（トップ・MiniJinja テンプレート・静的 HTML）の作成・編集・削除と URL 公開
...
`http://127.0.0.1:3000/admin` にアクセスすると管理ダッシュボードが表示されます。`http://127.0.0.1:3000/admin/posts` からお知らせ、`http://127.0.0.1:3000/admin/pages` からページ（トップ `index.html` を含む）を作成・編集できます。

- 認証は未実装のため、ログインなしで開けます
- メディア・ユーザー・設定・コメントなどは未実装です
...
|-------------|--------|--------------|
| `0` | `work/templates/` | [MiniJinja](https://github.com/mitsuhiko/minijinja) で評価（`blogname` / `blogdescription` / `news` を渡す） |
...
- **管理画面**: `/admin/pages` で一覧・編集。初回起動後は「トップページ」行から `index.html` を編集できます。
