...
async fn get_one(State(state): State<AppState>, Path(id): Path<i64>) -> ApiResult<Json<Post>> {
    let post = services::posts::find(&state.pool, id).await.map_err(ApiError::from)?;
    Ok(Json(post))
}
...
async fn update(
    ...
    let current = services::posts::find(&state.pool, id).await.map_err(ApiError::from)?;
    ...
    let updated = services::posts::find(&state.pool, id).await.map_err(ApiError::from)?;
    Ok(Json(updated))
}

async fn delete_one(State(state): State<AppState>, Path(id): Path<i64>) -> ApiResult<Json<serde_json::Value>> {
    services::posts::delete(&state.pool, id).await.map_err(ApiError::from)?;
